...
public:
    ...
    int findRecursive(const string& name) const { return searchRecursive(root, name)->originalIndex; }
    int findIterative(const string& name) const { return searchIterative(root, name)->originalIndex; }
